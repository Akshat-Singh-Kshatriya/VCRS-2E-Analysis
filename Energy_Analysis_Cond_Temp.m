...
for r = 1:length(refrigerants)
    ...
    fprintf('%-10s | %-12s | %-12s | %-12s | %-12s | %-12s\n', ...
            'T_cond (K)', 'VCRS COP', 'Ejector COP', 'COP Imp (%)', 'VCRS W (kW)', 'Ej W (kW)');
    fprintf('%s\n', repmat('-', 1, 85));
    
    for i = 1:n
        cop_imp = ((results_store.(ref_field).COP_ej(i) - results_store.(ref_field).COP_vcrs(i)) / results_store.(ref_field).COP_vcrs(i)) * 100;
        
        fprintf('%-10.2f | %-12.3f | %-12.3f | %-12.2f | %-12.3f | %-12.3f\n', ...
            T_cond_array(i), ...
            results_store.(ref_field).COP_vcrs(i), ...
            results_store.(ref_field).COP_ej(i), ...
            cop_imp, ...
            results_store.(ref_field).W_vcrs(i), ...
            results_store.(ref_field).W_ej(i));
    end
end
...
fixed_param = sprintf('T_{evap} = %.2f K', T_evap);
