...
for r = 1:length(refrigerants)
    ...
    fprintf('\n%s Results\n', ref);
    fprintf('%-10s | %-12s | %-12s | %-12s | %-12s\n', 'Cond Temp` (K)', 'VCRS eta_ex', 'Ej eta_ex', 'VCRS EDR', 'Ej EDR');
    fprintf('%s\n', repmat('-', 1, 65));
...
create_combined_plot(T_cond_array, ...
    ...
    'Condenser Temperature (K)', 'Exergy Efficiency (\eta_{ex})', ...
    'R1234yf Simple VCRS', 'R1234yf Ejector VCRS', ...
    'R1234ze(E) Simple VCRS', 'R1234ze(E) Ejector VCRS', ...
    fixed_param, c_ref1_vcrs, c_ref1_ej, c_ref2_vcrs, c_ref2_ej, 'Combined_Tcond_ExergyEff.png');

create_combined_plot(T_cond_array, ...
    results_store.(ref1).EDR_vcrs, results_store.(ref1).EDR_ej, ...
    results_store.(ref2).EDR_vcrs, results_store.(ref2).EDR_ej, ...
    'Condenser Temperature (K)', 'Exergy Destruction Ratio (EDR)', ...
    'R1234yf Simple VCRS', 'R1234yf Ejector VCRS', ...
    'R1234ze(E) Simple VCRS', 'R1234ze(E) Ejector VCRS', ...
    fixed_param, c_ref1_vcrs, c_ref1_ej, c_ref2_vcrs, c_ref2_ej, 'Combined_Tcond_EDR.png');
